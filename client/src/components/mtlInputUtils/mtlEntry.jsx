...
function MtlEntry() {
  ...
  const handleSubmit = (e) => {
    ...
    setStatus("crawling");
    axios
      .post(`${process.env.REACT_APP_API_URL}/crawl/mtl`, entry)
      .then((res) => {
        setData(res.data);
        setStatus("crawled");
      })
    ...
    <>
      ...
      <section className="faloo text-[#494949] bg-[#ebebeb] min-h-[100vh]">
        <div className="max-w-[900px] mx-auto py-[20px]">
          <h1 className="text-[35px] capitalize">
            MTL - Scraping Faloo Website
          </h1>
        ...
        <div className="fixed bottom-0 left-0 w-full bg-white shadow border-t px-[25px] items-center h-[25px] flex text-[14px] font-[400]">
          <Link to="/">Home</Link>
          <span className="mx-[3px]"> ➛ </span>
          <Link to="/home">Faloo</Link>
          <span className="mx-[3px]"> ➛ </span>
          <Link onClick={postData}>Post Data</Link>
        </div>
      </section>
    </>
  );
