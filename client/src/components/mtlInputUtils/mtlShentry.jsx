...
function MtlShEntry() {
  ...
  const handleSubmit = (e) => {
    ...
    setStatus("crawling");
    axios
      .post(`${process.env.REACT_APP_API_URL}/crawl/mtlshoudashu`, entry)
      .then((res) => {
        setData(res.data);
        setStatus("crawled");
      })
  ...
  return (
    <>
      <section className="shoudashu text-[#494949] min-h-[100vh] bg-[#ebebeb]">
        <div className="max-w-[900px] mx-auto py-[20px]">
          <h1 className="text-[35px] capitalize">MTL - Scraping Shoudashu Website</h1>
          <div className="my-5 bg-white border shadow rounded-lg p-10 min-h-[650px] leading-[1]">
            <div className="info max-w-[600px] mx-auto">
              <h2 className="pt-[35px] font-[500] text-[250%] leading-[1] text-center mb-[25px]">
                Scrape shoudashu novels with this scraping tool.
              </h2>
              <p className="text-[20px] text-[#7c7c7c] leading-[1.2] text-center mb-[20px]">
                ...
                from www.shoudashu.com. With the immense popularity of faloo novels,
            ...
            <div className="fixed bottom-0 left-0 w-full bg-white shadow border-t px-[25px] items-center h-[25px] flex text-[14px] font-[400]"><Link to="/">Home</Link><span className="mx-[3px]"> ➛ </span><Link to="/shoudashu">Shoudashu</Link><span className="mx-[3px]"> ➛ </span><Link onClick={postData}>Post Data</Link></div>
          </div>
        </div>
      </section>
    </>
